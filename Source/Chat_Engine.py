...
class ChatEngine:
    ...
    def _load_categories(self):
        """Load categories from the Resources/categories.txt file."""
        categories_file = os.path.join("Resources", "categories.txt")
    ...
    def detect_categories(self, query):
        ...
        Matches query words against the categories file generated by the scraper.
        Returns an empty list if no match is found (search spans all categories).

        Args:
            query: The user's natural language question.

        Returns:
            list[str]: Matching category names from categories.txt.
        """
        query_lower = query.lower()
        categories = self._load_categories()
        detected = [cat for cat in categories if cat in query_lower]
